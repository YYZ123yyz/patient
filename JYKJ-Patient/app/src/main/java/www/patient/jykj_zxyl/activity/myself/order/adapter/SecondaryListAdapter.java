...
//                    }
     ...
     */
    public void expand(int groupItemIndex){
        final DataTree dt = dataTrees.get(groupItemIndex);
        groupItemStatus.set(groupItemIndex, true);
        notifyItemRangeInserted(groupItemIndex, dt.getSubItems().size());
    }
     ...
     * @param groupItemIndex
     */
    public void fold(int groupItemIndex){
        final DataTree dt = dataTrees.get(groupItemIndex);
        groupItemStatus.set(groupItemIndex, false);
        notifyItemRangeRemoved(groupItemIndex, dt.getSubItems().size());
    }
    ...
    public final int getItemCount() {
        ...
        for (int i = 0; i < dataTrees.size(); i++) {
            ...
            if (groupItemStatus.get(i)) {
                if (dataTrees.get(i).getSubItems()!=null) {
