...
public class SharedPreferences_DataSave {
    ...
    private final static String TAG = "JYKJ.DocterApp.persist";
    ...
    public SharedPreferences_DataSave(Context ctx, String storeName) {
        this.storeName = storeName;
        this.seed = "jykjdocter+" + this.storeName + "-jykjdocter";
        this.sharedPrefs = ctx.getSharedPreferences(this.seed,
    ...
    public void putString(String key, String value) {
        // only for v1.5 (Bug in 1.1 crashes prefs, see
        // http://code.google.com/p/android/issues/detail?id=2354)
        if (value == null)
            getEditor().remove(key);
        else
