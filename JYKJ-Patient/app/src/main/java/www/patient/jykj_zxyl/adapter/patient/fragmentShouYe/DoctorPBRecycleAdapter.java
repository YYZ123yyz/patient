 ...
 * 医生排班
 */
public class DoctorPBRecycleAdapter extends RecyclerView.Adapter<DoctorPBRecycleAdapter.ViewHolder> {
    public          List<ProvideDoctorSetSchedulingPatient>            datas = new ArrayList<>();
    private         OnItemClickListener     mOnItemClickListener;           //早上点击事件

    private         Context                 mContext;


    public DoctorPBRecycleAdapter(List<ProvideDoctorSetSchedulingPatient> list, Context context){
        mContext = context;
        datas = list;
    }

    //重新设置数据
    public void setDate(List<ProvideDoctorSetSchedulingPatient> list){
        datas = list;
    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder, final int position) {
            ...
            viewHolder.mDate.setText(datas.get(position).getText());
