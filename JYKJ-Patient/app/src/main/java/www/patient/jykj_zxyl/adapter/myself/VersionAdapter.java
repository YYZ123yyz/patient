...
public class VersionAdapter extends RecyclerView.Adapter<VersionAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(ViewHolder holder, int i) {
        ...
        holder.dateShow.setText(datas.get(i).getUpdateDateShow());
        holder.num.setText(datas.get(i).getVersionNum());
        if (datas.get(i).getThisTimeUpdDesc().indexOf("\\n") >= 0) {
            String str2 = datas.get(i).getThisTimeUpdDesc().replace("\\n", " \n ");
            holder.ver_this.setText(str2);
        }

        if (datas.get(i).getLatelyUpdDesc().indexOf("\\n") >= 0) {
            String str = datas.get(i).getLatelyUpdDesc().replace("\\n", " \n ");
