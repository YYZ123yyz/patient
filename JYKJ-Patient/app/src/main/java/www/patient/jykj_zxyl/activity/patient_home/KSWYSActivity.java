...
public class KSWYSActivity extends AppCompatActivity {
    ...
    private void setLayoutDate() {
        ...
        //设置初始数据
        mProvideInteractPatientInterrogation.setBloodPressureAbnormalDate(mProvideInteractPatientInterrogations.get(0).getBloodPressureAbnormalDate());
        mProvideInteractPatientInterrogation.setHtnHistory(mProvideInteractPatientInterrogations.get(0).getHtnHistory());
        mProvideInteractPatientInterrogation.setFlagFamilyHtn(mProvideInteractPatientInterrogations.get(0).getFlagFamilyHtn());
        mProvideInteractPatientInterrogation.setHighPressureNum(mProvideInteractPatientInterrogations.get(0).getHighPressureNum());
        mProvideInteractPatientInterrogation.setLowPressureNum(mProvideInteractPatientInterrogations.get(0).getLowPressureNum());
        mProvideInteractPatientInterrogation.setHeartRateNum(mProvideInteractPatientInterrogations.get(0).getHeartRateNum());
        mProvideInteractPatientInterrogation.setMeasureInstrument(mProvideInteractPatientInterrogations.get(0).getMeasureInstrument());
        mProvideInteractPatientInterrogation.setMeasureInstrumentName(mProvideInteractPatientInterrogations.get(0).getMeasureInstrumentName());
        mProvideInteractPatientInterrogation.setMeasureMode(mProvideInteractPatientInterrogations.get(0).getMeasureMode());
        mProvideInteractPatientInterrogation.setMeasureModeName(mProvideInteractPatientInterrogations.get(0).getMeasureModeName());
        mProvideInteractPatientInterrogation.setStateOfIllness(mProvideInteractPatientInterrogations.get(0).getStateOfIllness());
        ...
        if (mProvideInteractPatientInterrogations.get(0).getGender() == 2)
            mXB.setText("女");
        mProvideInteractPatientInterrogation.setGender(mProvideInteractPatientInterrogations.get(0).getGender());
...
//            mYCRQ.setText(mProvideInteractPatientInterrogations.get(0).getBloodPressureAbnormalDate());
...
//            mProvideInteractPatientInterrogation.setBloodPressureAbnormalDate(mYear+"-"+month+"-"+day);
            System.out.println();
        }
    };
    ...
    private void showCLFSDialog() {
        ...
        builder3.setSingleChoiceItems(items, 0, new DialogInterface.OnClickListener() {// 2默认的选中
            ...
            public void onClick(DialogInterface dialog, int which) {// which是被选中的位置
                ...
                mProvideInteractPatientInterrogation.setMeasureMode(mApp.gBasicDate.get(10007).get(which).getBaseCode());
                mProvideInteractPatientInterrogation.setMeasureModeName(mApp.gBasicDate.get(10007).get(which).getAttrName());
    ...
    private void showCLYQDialog() {
        ...
        builder3.setSingleChoiceItems(items, 0, new DialogInterface.OnClickListener() {// 2默认的选中
            ...
            public void onClick(DialogInterface dialog, int which) {// which是被选中的位置
                ...
                mProvideInteractPatientInterrogation.setMeasureInstrument(mApp.gBasicDate.get(10006).get(which).getBaseCode());
                mProvideInteractPatientInterrogation.setMeasureInstrumentName(mApp.gBasicDate.get(10006).get(which).getAttrName());
                dialog.dismiss();// 随便点击一个item消失对话框，不用点击确认取消
            }
        });
        builder3.show();// 让弹出框显示
    ...
    private void showDoctorTitleDialog() {
        ...
        builder3.setSingleChoiceItems(doctorTitleName, 0, new DialogInterface.OnClickListener() {// 2默认的选中
            ...
            public void onClick(DialogInterface dialog, int which) {// which是被选中的位置
                ...
                mProvideInteractPatientInterrogation.setFlagFamilyHtn(which);
                dialog.dismiss();// 随便点击一个item消失对话框，不用点击确认取消
            }
        });
        builder3.show();// 让弹出框显示
    ...
    private void showChoiceNLDialog() {
        ...
        builder3.setSingleChoiceItems(doctorTitleName, 0, new DialogInterface.OnClickListener() {// 2默认的选中
            ...
            public void onClick(DialogInterface dialog, int which) {// which是被选中的位置
                ...
                mProvideInteractPatientInterrogation.setGender(which+1);
                dialog.dismiss();// 随便点击一个item消失对话框，不用点击确认取消
            }
        });
        builder3.show();// 让弹出框显示
...
//        mProvideInteractPatientInterrogation = new ProvideInteractPatientInterrogation();
        mProvideInteractPatientInterrogation.setLoginPatientPosition(mApp.loginDoctorPosition);
        mProvideInteractPatientInterrogation.setRequestClientType("1");
        mProvideInteractPatientInterrogation.setOperPatientCode(mApp.mProvideViewSysUserPatientInfoAndRegion.getPatientCode());
        mProvideInteractPatientInterrogation.setOperPatientName(mApp.mProvideViewSysUserPatientInfoAndRegion.getUserName());
        mProvideInteractPatientInterrogation.setOrderCode(mOrderNum);
        if (mProvideInteractPatientInterrogation.getOrderCode() == null || "".equals(mProvideInteractPatientInterrogation.getOrderCode()))
        {
            Toast.makeText(mContext,"订单号获取失败，请稍后重试",Toast.LENGTH_SHORT).show();
            return;
        }
        mProvideInteractPatientInterrogation.setTreatmentType(Integer.valueOf(mOperaType));
        if (mProvideInteractPatientInterrogation.getTreatmentType() == null || "".equals(mProvideInteractPatientInterrogation.getTreatmentType()))
        {
        ...
        mProvideInteractPatientInterrogation.setPatientCode(mApp.mProvideViewSysUserPatientInfoAndRegion.getPatientCode());
        mProvideInteractPatientInterrogation.setPatientName(mApp.mProvideViewSysUserPatientInfoAndRegion.getUserName());
        mProvideInteractPatientInterrogation.setPatientLinkPhone(mHZSJ.getText().toString());
//        mProvideInteractPatientInterrogation.setGender(mApp.mProvideViewSysUserPatientInfoAndRegion.getGender());
        mProvideInteractPatientInterrogation.setHtnHistory(mGXYBS.getText().toString());
        ...
        mProvideInteractPatientInterrogation.setHighPressureNum(Integer.valueOf(mSZY.getText().toString()));
        if (mProvideInteractPatientInterrogation.getHighPressureNum() == null || "".equals(mProvideInteractPatientInterrogation.getHighPressureNum()))
        {
            Toast.makeText(mContext,"请填写舒张压",Toast.LENGTH_SHORT).show();
            return;
        }
        mProvideInteractPatientInterrogation.setLowPressureNum(Integer.valueOf(mSSY.getText().toString()));
        if (mProvideInteractPatientInterrogation.getLowPressureNum() == null || "".equals(mProvideInteractPatientInterrogation.getLowPressureNum()))
        {
            Toast.makeText(mContext,"请填写收缩压",Toast.LENGTH_SHORT).show();
            return;
        }
        mProvideInteractPatientInterrogation.setHeartRateNum(Integer.valueOf(mXL.getText().toString()));
        if (mProvideInteractPatientInterrogation.getHeartRateNum() == null || "".equals(mProvideInteractPatientInterrogation.getHeartRateNum()))
        {
            Toast.makeText(mContext,"请填写心率",Toast.LENGTH_SHORT).show();
            return;
        }
        mProvideInteractPatientInterrogation.setStateOfIllness(mBQZS.getText().toString());
        if (mProvideInteractPatientInterrogation.getStateOfIllness() == null || "".equals(mProvideInteractPatientInterrogation.getStateOfIllness()))
        {
            Toast.makeText(mContext,"请填写病情自述",Toast.LENGTH_SHORT).show();
            return;
        }
        if (mProvideInteractPatientInterrogation.getBloodPressureAbnormalDate() == null || "".equals(mProvideInteractPatientInterrogation.getBloodPressureAbnormalDate()))
        {
            Toast.makeText(mContext,"请选择最早发现异常日期",Toast.LENGTH_SHORT).show();
            return;
        }
        if (mProvideInteractPatientInterrogation.getFlagFamilyHtn() == null || "".equals(mProvideInteractPatientInterrogation.getFlagFamilyHtn()))
        {
            Toast.makeText(mContext,"请选择家族内是否有高血压患者",Toast.LENGTH_SHORT).show();
            return;
        }
        mProvideInteractPatientInterrogation.setBirthday(mNL.getText().toString());
        if (mProvideInteractPatientInterrogation.getMeasureInstrument() == null || "".equals(mProvideInteractPatientInterrogation.getMeasureInstrument()))
        {
            Toast.makeText(mContext,"请选择测量仪器",Toast.LENGTH_SHORT).show();
            return;
        }
        if (mProvideInteractPatientInterrogation.getMeasureMode() == null || "".equals(mProvideInteractPatientInterrogation.getMeasureMode()))
        {
...
//                //提交数据
                try {
                    String str = new Gson().toJson(mProvideInteractPatientInterrogation);
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what)
                {
                    ...
                    case 1:
                        ...
                        //如果不是图文就诊，则继续获取医生排班信息
                        if (!mOperaType.equals("1"))
                            getDoctorPBInfo();
                        else
                            cacerProgress();
                        break;
