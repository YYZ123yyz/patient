 ...
 * 首页==》推荐专家==>专家评价
 */
public class ActivityTJZJ_PJAdapter extends RecyclerView.Adapter<ActivityTJZJ_PJAdapter.ViewHolder> {
    ...
    public ActivityTJZJ_PJAdapter(List<ProvideViewDoctorExpertRecommend> list, Context context){
        mContext = context;
        datas = list;
    }
    ...
    public void setDate(List<ProvideViewDoctorExpertRecommend> list){
       ...
       @Override
        public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType){
                View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_fragmentshouye_tjzj,viewGroup,false);
    ...
    public void onBindViewHolder(ViewHolder viewHolder, final int position) {
        ...
        if (datas.get(position).getFlagImgText() == 1)
            viewHolder.mImgTWJZ.setBackgroundResource(R.mipmap.wdzs_twjz);
        else
            viewHolder.mImgTWJZ.setBackgroundResource(R.mipmap.tw_no);

        if (datas.get(position).getFlagAudio() == 1)
            viewHolder.mImgDHJZ.setBackgroundResource(R.mipmap.wdzs_dhjz);
        else
            viewHolder.mImgDHJZ.setBackgroundResource(R.mipmap.dh_no);

        if (datas.get(position).getFlagAudio() == 1)
            viewHolder.mImgYYJZ.setBackgroundResource(R.mipmap.yyjz);
        else
            viewHolder.mImgYYJZ.setBackgroundResource(R.mipmap.yy_no);

        if (datas.get(position).getFlagVideo() == 1)
            viewHolder.mImgSPJZ.setBackgroundResource(R.mipmap.wdzs_spjz);
        else
            viewHolder.mImgSPJZ.setBackgroundResource(R.mipmap.sp_no);

        if (datas.get(position).getFlagSigning() == 1)
            viewHolder.mImgQYJZ.setBackgroundResource(R.mipmap.qyjz);
        else
            viewHolder.mImgQYJZ.setBackgroundResource(R.mipmap.qy_no);
