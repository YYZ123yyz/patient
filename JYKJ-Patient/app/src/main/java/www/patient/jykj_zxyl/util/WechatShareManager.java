...
import www.patient.jykj_zxyl.util.widget.Utils;

/**
 * Company    : Test
 * Author     : 杨运泽
 * Date       : 2020/1/23 0023 11:13
 * 实现微信分享功能的核心类
 */
public class WechatShareManager {
    ...
    private ShareContent mShareContentText, mShareContentPicture, mShareContentWebpag, mShareContentVideo, mShareContentOther;
     ...
     * 获取图片分享对象
     */
    public ShareContent getShareContentPicture(int pictureResource, Bitmap bt) {
        this.bt = bt;
     ...
     * 获取视频分享内容
     */
    public ShareContent getShareContentOther(String url) {
        if (mShareContentOther == null) {
            //未完成
            mShareContentOther = new ShareContentVideo(url);
        }
        return (ShareContentVideo) mShareContentVideo;
    }
    ...
    private void sharePicture(ShareContent shareContent, int shareType) {
        Bitmap bitmap = bt;
        WXImageObject imgObj = new WXImageObject(bt);
     ...
     * 分享其他
     */
    private void shareoTHER(ShareContent shareContent, int shareType) {
        WXWebpageObject webpage = new WXWebpageObject();
